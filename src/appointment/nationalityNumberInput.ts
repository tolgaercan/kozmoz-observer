...
async function triggerNationalityValidationBlur(
  ...
  if (!settings.nationalityNumberBlankClickEnabled) {
    logger.info("TC doğrulama boş tık kapalı — NATIONALITY_NUMBER_BLANK_CLICK_ENABLED=false");
    return;
  }
  ...
  logger.info("TC doğrulama boş tık tamamlandı.");
}
...
export async function fillNationalityNumber(
  ...
  if (!settings.nationalityNumberEnabled) {
    logger.info("TC Kimlik girişi kapalı — NATIONALITY_NUMBER_ENABLED=false");
  ...
  if (!nationalityNumber) {
    throw new Error(
      `${profile.id} için nationalityNumber tanımlı değil (manifest veya NATIONALITY_NUMBER).`,
    );
  }

  if (nationalityNumber.length !== TC_DIGIT_COUNT) {
    throw new Error(`TC Kimlik No 11 haneli olmalı (${profile.id}).`);
  }

  if (!isValidTurkishNationalId(nationalityNumber)) {
    throw new Error(`TC Kimlik No geçersiz (${profile.id}, ${maskNationalityNumber(nationalityNumber)}).`);
  }
  ...
  logger.info(`TC Kimlik No giriliyor: ${masked} (profil: ${profile.id})`);
  ...
  for (const selector of locators) {
    ...
    try {
      ...
      if (entered !== nationalityNumber) {
        throw new Error(
          `Alan değeri doğrulanamadı (beklenen: ${masked}, okunan: ${maskNationalityNumber(entered)}).`,
        );
      }

      logger.info(`TC Kimlik No girildi: ${masked}`);
      await triggerNationalityValidationBlur(page, settings);
      return nationalityNumber;
    } catch (error) {
      lastError = error;
      logger.warn(
        `TC Kimlik locator başarısız (${selector}): ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }

  throw new Error(
    `TC Kimlik No girilemedi (${masked}). Denenen: ${locators.join(" | ")}`,
