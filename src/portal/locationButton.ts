...
/** Panel ofis adı ile eşleşen merkez/şube kartına insan gibi tıkla (wizard adım 2). */
export async function clickAppointmentLocationButton(
  ...
  logger.info(`Merkez/şube butonu aranıyor (isim): ${officeName}`);

  const { locator, label } = await resolveLocationButton(page, officeName, settings);
